...
namespace eWoW.Network.Packets
{
    public class PacketWriter : BinaryWriter
    {
        ...
        public byte[] PacketData
        {
            get
            {
                byte[] tmp = (BaseStream as MemoryStream).ToArray();
                var ret = new byte[tmp.Length - (byte) _headerType];
                Array.Copy(tmp, (byte) _headerType, ret, 0, tmp.Length - (byte) _headerType);
                return ret;
            }
        }
        ...
        public void WritePacketHeader(ushort opcode, ushort length)
        {
        ...
        /// </summary>
        /// <param name="opcode">The opcode.</param>
        public void WritePacketHeader(ushort opcode)
        {
            // Awesomely lazy!
            var length = (ushort) (BaseStream.Length - (byte) _headerType);
